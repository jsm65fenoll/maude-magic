...
# %% auto 0
__all__ = ['TIMEOUT', 'TRACE', 'TimeoutException', 'MaudeShell', 'MaudeSystem', 'MaudeMagics', 'load_ipython_extension']
...
class MaudeShell:
    ...
    def _sync(self,timeout=TIMEOUT)->str:
        ...
        i = self.sh.expect([pexpect.TIMEOUT,'\rMaude> '],timeout=-1)
        if i == 0:
    ...
    def __call__(self,command,timeout=TIMEOUT,raises=True,filter=False):
        ...
        return self.__checkForMessages(response,raises,filter)
        

    def __checkForMessages(self,response,raises=True,filter=True):
        """ Scans response for Advisory or Warning message lines.
            If filter = True, drops not message lines.
        ...
        for line in response.strip().split("\n"): 
            line=line.replace("\r","")
            if TRACE: print("\tLine:",repr(line))
            match = self.messages_reo.match(line)
            if not match:
                if not filter: result += line+"\r\n"
            else:
...
class MaudeSystem(MaudeShell):
    ...
    def _getTextResult(self,command,term)->str:
        ...
        return self(command +" " + term.strip() + " .")
    ...
    def _getTupleResult(self,command,term,pattern,sort=None,value=None)->dict:
        """ Executes 'command' over 'term' and uses 'pattern' to fetch result components into a dict witth keys 
            type and value."""
        response = self(command +" " + term.strip()+ " .")
